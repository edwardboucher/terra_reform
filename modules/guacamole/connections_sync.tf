  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      CMD_ID=$(aws ssm send-command --region "$REGION" \
        ...
        --parameters commands="cd /guacamole-docker-compose && aws s3 cp $S3_URI . && ./guac_add_connections.sh '${var.guac_admin_username}' '${var.guac_admin_password}' ./connections.json" \
